public eventlib_Start()
{
	...
	else HookEvent("round_start", eventlib_RoundStart);
	...
	if ((g_vip_type == 2 || g_vip_type == 4 || g_vip_type == 6 || g_vip_type == 7) && GetClientCount() >= g_min_player)
	{
		for (new i = 1; i <= MaxClients; i++)
		{
			...
			if (clientlib_isValidClient(client))
			{
				...
				{
					pointlib_GivePlayerPoints(client);
					clientlib_CheckVip(client);
				}
			}
		}
	}
	if (g_happyhouron) CPrintToChatAll("%s %T", g_StammTag, "HappyActive", LANG_SERVER, g_points);
}
